// apps/wms-desktop/renderer/src/pages/ExternalInboundPage.jsx
...
import { externalInboundMode } from "../workflows/externalInbound/externalInbound.workflow";
import { jobStoreLabel, loadStores } from "../workflows/_common/storeMap";

const PAGE_KEY = "externalInbound";

export default function ExternalInboundPage({ pageTitle = "외부 입고", defaultStoreCode = "HQ" }) {
  const mode = externalInboundMode;
  ...
  const [scanLoc, setScanLoc] = useState(() => mode.defaultLocationCode || "IN-01");
  ...
  async function loadJobsFromServer() {
    try {
      ...
      // INBOUND 타입만 필터링
      const list = normalized.filter((j) => j.type === 'INBOUND');
      setCreated(list);
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <div style={{ fontSize: 12, color: "#64748b", marginTop: 6 }}>외부 입고: 공급업체에서 창고로 입고. 기본 locationCode는 <b>{mode.defaultLocationCode || "IN-01"}</b></div>
      <JobsRow />
      <div style={{ marginTop: 12, border: "1px solid #e5e7eb", borderRadius: 12, padding: 12, background: "#fff" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}>
