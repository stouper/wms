...
export default function StoreOutboundPage({ pageTitle = "매장 출고", defaultStoreCode = "" }) {
              ...
              >
                ...
                >
                  ...
                  <div style={{ fontSize: 12, color: "#64748b", whiteSpace: "nowrap" }}>
                    store <b>{jobStoreLabel(j, defaultStoreCode)}</b> · <b>{j.status}</b> · 등록: <b>{j.operatorId || "-"}</b>
                  </div>
                </button>
  ...
  function JobDetail() {
    ...
    const progressPct = totalPlanned > 0 ? Math.round((totalPicked / totalPlanned) * 100) : 0;

    return (
      <div style={{ marginTop: 12, border: "1px solid #e5e7eb", borderRadius: 12, padding: 12, background: "#fff" }}>
        <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}>
          <div style={{ fontWeight: 900, display: "flex", alignItems: "center", gap: 12 }}>
            선택된 Job 상세
            <span style={{
              fontSize: 13,
              fontWeight: 700,
              color: progressPct >= 100 ? "#059669" : "#1d4ed8",
              background: progressPct >= 100 ? "#ecfdf5" : "#eff6ff",
              padding: "2px 8px",
              borderRadius: 6
            }}>
              {progressPct}% ({totalPicked}/{totalPlanned})
            </span>
          </div>
          <div style={{ fontSize: 12, color: "#64748b" }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 15 }}>
              <thead>
                <tr style={{ background: "#f8fafc" }}>
                  ...
                  <Th align="center" style={{ width: 60 }}>출고</Th>
                  <Th align="center" style={{ width: 55 }}>진행</Th>
                  <Th align="right" style={{ width: 120 }}>추가</Th>
                </tr>
              </thead>
              <tbody>
                {items.map((it) => {
                  ...
                  return (
                    <tr key={it.id} style={{ background: isDone ? "#f0fdf4" : "transparent" }}>
                      ...
                      <Td style={{ fontSize: 15, fontWeight: 600, color: "#1e293b" }}>
                        {it?.sku?.name || it?.sku?.sku || it.skuCode || it.id}
                      </Td>
                      <Td style={{ fontSize: 15, fontWeight: 600, color: "#1e293b" }}>
                        {it?.sku?.makerCode || it.makerCodeSnapshot || "-"}
                      </Td>
                      <Td align="center" style={{ fontSize: 15, fontWeight: 600, color: "#1e293b" }}>{planned}</Td>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            <button
              ...
              onClick={async () => {
                ...
                // ✅ 전체취소는 음수 발생 가능성이 높으므로 강제 옵션 제공
                const confirmMsg = `이 Job의 스캔 로그를 전부 취소(UNDO ALL)할까?\n\n⚠️ 음수 재고가 발생할 수 있습니다.`;
                ...
                try {
                  ...
                  pushToast({ kind: "warn", title: "UNDO ALL", message: "전체 취소 완료 (음수 재고 발생 가능)" });
                  triggerTotalsFlash();
                  await loadJob(selectedJobId);
                  await loadTx(selectedJobId);
                  playScanSuccessBeep();
                  ...
                  {scanTxLogs.map((tx, idx) => {
                    ...
                    return (
                      <tr key={tx.id || idx}>
                        ...
                        <Td align="right" style={{ fontSize: 13, fontWeight: 700, color: "#1e293b" }}>
                          {qty}
                        </Td>
                        <Td align="right">
                          {/* ✅ 최근 스캔에만 취소 버튼 표시 */}
                          {isLatest ? (
                            <button
                              ...
                              onClick={async () => {
                                ...
                                let forceUndo = false;
                                try {
                                  const checkRes = await jobsFlow.checkUndo({ jobId: selectedJobId });
                                  if (checkRes?.hasNegative) {
                                    const items = checkRes.negativeItems || [];
                                    const details = items.map(it =>
                                      `• ${it.skuName || it.skuCode} (${it.locationCode}): ${it.currentQty} → ${it.afterQty}`
                                    ).join("\n");

                                    const confirmMsg = `⚠️ 경고: UNDO 시 재고가 음수가 됩니다!\n\n${details}\n\n음수 재고를 허용하고 강제 취소할까요?`;
                                    forceUndo = window.confirm(confirmMsg);
                                    if (!forceUndo) return;
                                  } else {
                                ...
                                try {
                                  ...
                                  pushToast({
                                    kind: forceUndo ? "warn" : "info",
                                    title: "UNDO",
                                    message: forceUndo ? "강제 취소 완료 (음수 재고 발생)" : "취소 완료 (1건)"
                                  });
                                  triggerTotalsFlash();
                                  await loadJob(selectedJobId);
                                  await loadTx(selectedJobId);
